...
function normalizeProduct(p){p.image=/^data:image\/(jpeg|png|webp|gif);base64,/i.test(p.image||"")?p.image:"";p.rounding=Number(p.rounding)||1000;p.targetPrice=Number(p.targetPrice)||0;p.costing={...defaults(),...(p.costing||{})};p.listing={title:p.listing?.title??p.name??"",description:p.listing?.description??""};p.components=(p.components||[]).map(c=>({...c,printHours:Number(c.printHours)||0}));return p;}
...
function templateDefaults(){const source=state.templateDefaults||{},fallback=state.products?.find(p=>p.id===currentProductId)||state.products?.[0];return {costing:{...defaults(),...(fallback?.costing||{}),...(source.costing||{})},rounding:Number(source.rounding||fallback?.rounding)||1000};}
function rememberTemplateDefaults(product){state.templateDefaults={costing:clone(product.costing),rounding:Number(product.rounding)||1000};}
...
async function copyText(text){try{await navigator.clipboard.writeText(text);toast("Đã sao chép");}catch(error){const area=document.createElement("textarea");area.value=text;area.style.position="fixed";area.style.opacity="0";document.body.append(area);area.select();document.execCommand("copy");area.remove();toast("Đã sao chép");}}
...
function cloudPayload(){return {materials:clone(state.materials),products:state.products.map(({image,...p})=>({...clone(p),image:""})),templateDefaults:clone(state.templateDefaults||templateDefaults()),updatedAt:serverTimestamp(),version:3};}
...
async function applyCloudData(data,workspaceId){state={materials:Array.isArray(data.materials)?data.materials:[],products:Array.isArray(data.products)?data.products:[],templateDefaults:data.templateDefaults||null};state.products.forEach(normalizeProduct);await Promise.all(state.products.map(async p=>{let imageSnap=await getDoc(doc(db,"workspaces",workspaceId,"images",p.id));if(!imageSnap.exists()&&currentUser)imageSnap=await getDoc(doc(db,"users",currentUser.uid,"images",p.id));if(imageSnap.exists())p.image=normalizeProduct({...p,image:imageSnap.data().dataUrl}).image;}));persistLocal();updatePills();}
...
function renderProducts(filter=""){
  ...
  <div class="product-grid">${list.map((p,i)=>`<article class="product-card" data-product-id="${p.id}"><div class="product-cover ${p.image?"has-image":""}">${productVisual(p,"Chưa có ảnh")}</div><div class="product-card-body"><div class="product-card-top"><span class="product-index">MẪU ${String(i+1).padStart(2,"0")}</span><button class="card-menu" data-duplicate="${p.id}" title="Nhân bản">⧉</button></div><h3>${escapeHtml(p.name)}</h3><p>${p.components.length} cấu phần · ${number(totalPrintHours(p))} giờ máy</p><div class="card-stats"><div><strong>${money(productCost(p))}</strong><span>Giá thành đủ</span></div><div><strong>${money(suggestedPrice(p))}</strong><span>Giá gợi ý</span></div></div></div></article>`).join("")}<article class="product-card new-card" data-create-product><div><b>＋</b><h3>Tạo mẫu mới</h3><p>Bắt đầu từ một BOM trống</p></div></article></div>`;
}

function createProduct(){const template=templateDefaults();const p=normalizeProduct({id:uid(),name:"Mẫu mới",rounding:template.rounding,targetPrice:0,costing:clone(template.costing),components:[{id:uid(),name:"Cấu phần 1",materialId:state.materials[0]?.id||"",quantity:1,printHours:0}]});state.products.push(p);saveState();navigate("editor",p.id);toast("Đã tạo mẫu mới với thông số mặc định");}
...
function renderEditor(){
  ...
  $("#editorView").innerHTML=`${pageHeading("CHỈNH ĐỊNH MỨC","Cân đối giá thành đầy đủ","Gram nhựa chỉ là một phần — thời gian máy và phụ phí cũng phải được trả tiền.",`<button class="secondary-button" data-back-products>← Trở lại</button>`)}
    ...
    <section class="cost-settings"><div class="cost-settings-head"><div><span class="eyebrow">THÔNG SỐ SẢN XUẤT</span><h3>Máy, nhân công và phụ phí</h3></div><p>Giá trị bạn chỉnh tại đây sẽ tự dùng cho mẫu tạo mới tiếp theo.</p></div>
    ...
    <section class="listing-section"><div class="listing-head"><div><span class="eyebrow">NỘI DUNG ĐĂNG BÁN</span><h3>Dành cho cộng tác viên</h3></div><p>Chỉnh nội dung một lần, cộng tác viên chỉ việc bấm sao chép và đăng bài.</p></div>
      <div class="listing-copy-grid"><div class="copy-field price-copy"><label>Giá đăng bán</label><div><input id="copySalePrice" readonly value="${escapeAttr(money(p.targetPrice||suggestedPrice(p)))}"><button type="button" data-copy-target="#copySalePrice">Sao chép</button></div></div>
      <div class="copy-field"><label>Tiêu đề mẫu</label><div><input id="copyListingTitle" data-listing-field="title" value="${escapeAttr(p.listing.title)}" placeholder="Ví dụ: Đèn ngủ in 3D phong cách hiện đại"><button type="button" data-copy-target="#copyListingTitle">Sao chép</button></div></div>
      <div class="copy-field description-copy"><label>Mô tả mẫu</label><div><textarea id="copyListingDescription" data-listing-field="description" rows="7" placeholder="Nhập mô tả sản phẩm, kích thước, màu sắc, tình trạng và cách liên hệ...">${escapeHtml(p.listing.description)}</textarea><button type="button" data-copy-target="#copyListingDescription">Sao chép</button></div></div></div>
    </section></div>
...
document.addEventListener("click",e=>{
  ...
  const copyButton=e.target.closest("[data-copy-target]");if(copyButton){const field=$(copyButton.dataset.copyTarget);copyText(field?.value||field?.textContent||"");return;}
...
document.addEventListener("input",e=>{
  ...
  if(e.target.id==="productName"){const oldName=p.name;p.name=e.target.value;if(!p.listing.title||p.listing.title===oldName){p.listing.title=e.target.value;const titleCopy=$("#copyListingTitle");if(titleCopy)titleCopy.value=e.target.value;}saveState();return;}
  if(e.target.id==="roundingInput"){p.rounding=Number(e.target.value)||1;rememberTemplateDefaults(p);saveState();updateEditorTotals(p);return;}
  if(e.target.id==="targetPriceInput"){p.targetPrice=Number(e.target.value)||0;saveState();updateEditorTotals(p);return;}
  if(e.target.dataset.costField){p.costing[e.target.dataset.costField]=Number(e.target.value)||0;rememberTemplateDefaults(p);saveState();updateEditorTotals(p);return;}
  if(e.target.dataset.listingField){p.listing[e.target.dataset.listingField]=e.target.value;saveState();return;}
